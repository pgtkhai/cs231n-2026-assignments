...
def sim(z_i, z_j):
    ...
    ##############################################################################
    dot_product = torch.dot(z_i.squeeze(), z_j.squeeze())
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        # 2. Numerators for l(k, k+N) and l(k+N, k)
        sim_pos = sim(z_k, z_k_N)
        num_k = torch.exp(sim_pos / tau)
        num_k_N = num_k  # sim(z_k_N, z_k) is symmetric
        
        # 3. Denominators (sum over all m != current anchor)
        denom_k = 0.0
        for m in range(2 * N):
            if m != k:
        ...
        for m in range(2 * N):
            if m != (k + N):
                denom_k_N += torch.exp(sim(z_k_N, out[m]) / tau)
                
        # 4. Losses l(k, k+N) and l(k+N, k)
        loss_k = -torch.log(num_k / denom_k)
        loss_k_N = -torch.log(num_k_N / denom_k_N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # HINT: torch.linalg.norm might be helpful.                                  #
    ##############################################################################
    # Row-wise dot product: (N, D) * (N, D) -> sum across D -> (N, 1)
    dot_product = (out_left * out_right).sum(dim=1, keepdim=True)

    # Row-wise L2 norms: (N, 1)
...
def compute_sim_matrix(out):
    ...
    # TODO: Start of your code.                                                  #
    ##############################################################################
    # 1. Normalize each vector to have unit L2 norm along feature dimension (dim=1)
    out_norm = torch.nn.functional.normalize(out, p=2, dim=1)

    # 2. Pairwise cosine similarity matrix: (2N, D) @ (D, 2N) -> (2N, 2N)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # Positive similarities are symmetric for (k, k+N) and (k+N, k)
    sim_pos_all = torch.cat([sim_pos, sim_pos], dim=0)  # [2*N, 1]
    
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = torch.exp(sim_pos_all / tau)  # [2*N, 1]
